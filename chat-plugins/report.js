...
exports.commands = {
	...
	requesthelp: function (target, room, user) {
		if (!user.can('pban')) return false; // check if a user cannot use the command rather than if they can
		...
		saveReports();
		// does not need a unique function simply because global.pmUpperStaff = pmUpperStaff
	...
	reports: function (target, room, user, connection) {
		...
		let params = cmdParts.join(' ').split(',').map(param => { return param.trim(); });
		let id = params.shift();
		switch (cmd) {
			case '':
			case 'view':
